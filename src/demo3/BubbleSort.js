...
import InnerBar from './../demo3/innerBar';
...
function SelectionSort() {
    ...
    const addData = (e) =>{
        e.preventDefault();
        let inputdata = [...data, ...input.split(',').map((ele) => parseInt(ele))];
        setdata(inputdata);
    ...
    const BubbleSort = () =>{
        ...
        {
            for(let j = 1; j < sortingData.length; j++)
            {
                if(sortingData[j] > sortingData[j - 1])
                {
                    tmplog.push([j, j - 1]);
                    [sortingData[j], sortingData[j - 1]] = [sortingData[j - 1], sortingData[j]];
                }
            }
        }
        setlog(tmplog);
    ...
    return (
        <div>
            <h1 className = 'px-4 py-2 font-weight-bold mb-0 bg-light-gray'>Bubble Sort</h1>
                        ...
                        <MDBCard className = 'p-3 h-100'>
                            ...
                            <MDBRow className = 'align-items-center justify-content-center'>
                                ...
                                <MDBCol  className = 'col-md-4 col-12 d-flex w-100'>
                                    <MDBBtn className = "bg-light-blue font-weight-bold d-flex flex-grow-1 px-0" onClick = {BubbleSort}><span className = 'mx-auto'>BubbleSort</span></MDBBtn>
